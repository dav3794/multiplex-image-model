...
class IBOTLoss(nn.Module):
    """Masked-token self-distillation over discrete decoder prototypes."""
    ...
    def forward(
        self,
        student_outputs: torch.Tensor | list[torch.Tensor],
        teacher_outputs: torch.Tensor | list[torch.Tensor],
        teacher_temp: float,
        view_token_counts: torch.Tensor | None = None,
    ) -> torch.Tensor:
        if isinstance(student_outputs, list):
            if view_token_counts is None:
                view_token_counts = torch.tensor(
                    [output.shape[0] for output in student_outputs],
                    device=student_outputs[0].device,
                )
            student_outputs = torch.cat(student_outputs)
            teacher_outputs = torch.cat(teacher_outputs)

        student = student_outputs.float().reshape(-1, student_outputs.shape[-1])
        teacher = teacher_outputs.float().reshape(-1, teacher_outputs.shape[-1])
        student_log = F.log_softmax(student / self.student_temp, dim=-1)
        teacher_prob = F.softmax(
            (teacher - self.center) / teacher_temp, dim=-1
        ).detach()
        token_losses = -(teacher_prob * student_log).sum(dim=-1)

        if view_token_counts is None:
            loss = token_losses.mean()
        else:
            view_token_counts = view_token_counts.to(
                device=token_losses.device, dtype=torch.long
            )
            view_indices = torch.repeat_interleave(
                torch.arange(view_token_counts.numel(), device=token_losses.device),
                view_token_counts,
                output_size=token_losses.shape[0],
            )
            view_losses = torch.zeros(
                view_token_counts.numel(),
                device=token_losses.device,
                dtype=token_losses.dtype,
            )
            view_losses.scatter_add_(0, view_indices, token_losses)
            loss = (view_losses / view_token_counts.clamp_min(1)).mean()
