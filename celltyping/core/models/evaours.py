...
def setup_evaours(
    ...
    model = EvaMAE(eva_conf)
    state_dict = torch.load(ckpt_path, map_location=device)["state_dict"]
    keys = list(state_dict.keys())
    for k in keys:
        state_dict[k.split(".", 1)[1]] = state_dict.pop(k)
    ...
    if scheme == "context":
        ...
        def prepare_fn(x_raw: torch.Tensor, mask_raw: torch.Tensor, tid: int, dataset):
            ...
            # CHANGED: Pad by half of 128 (64) so cells near the edge get full 128x128 context
            pad_size = 64
        ...
        def compute_fn(model, x, channels, mask, device, batch_size):
            ...
            x_batch = pad_image_to_target(x_batch, 128)
