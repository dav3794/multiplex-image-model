...
def zscore_stats_from_bags(
    bags: list[np.ndarray],
    indices: np.ndarray | None = None,
    eps: float = 1e-6,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Per-dimension mean and std across all crop rows in the selected bags.
    Used for z-score: (x - mu) / std.
    """
    if indices is None:
        chunks = bags
    else:
        chunks = [bags[int(i)] for i in indices]
    if len(chunks) == 0:
        raise ValueError("zscore_stats_from_bags: empty bag selection")
    stacked = np.concatenate(chunks, axis=0).astype(np.float64, copy=False)
    mu = stacked.mean(axis=0)
    std = np.maximum(stacked.std(axis=0), float(eps))
    return mu.astype(np.float32), std.astype(np.float32)


def apply_zscore_to_bags(
    bags: list[np.ndarray],
    mu: np.ndarray,
    std: np.ndarray,
) -> list[np.ndarray]:
    """Return new bags with shape-preserving z-score."""
    ...
    for b in bags:
        ...
        out.append(((arr - mu) / std).astype(np.float32))
    return out
...
def run(
    ...
    for dataset in datasets:
        ...
        for feature in features:
            ...
            bags_raw = bags
            if zscore_normalize == "global":
                global_mu, global_std = zscore_stats_from_bags(bags_raw)
                bags = apply_zscore_to_bags(bags_raw, global_mu, global_std)
                print(
                    "Z-score: computed mean/std over all pooled crop rows (global).",
                    flush=True,
                )
            elif zscore_normalize == "cv_train":
                bags = bags_raw
                print(
                    "Z-score: will fit mean/std on each CV train fold only (no leakage).",
                    flush=True,
                )
            elif zscore_normalize != "none":
                raise ValueError(
                    "zscore_normalize must be one of: none, global, cv_train"
                )
            ...
            for fold, (train_idx, val_idx) in enumerate(split_iter):
                ...
                if zscore_normalize == "cv_train":
                    fold_mu, fold_std = zscore_stats_from_bags(bags_raw, indices=train_idx)
                    bags_norm = apply_zscore_to_bags(bags_raw, fold_mu, fold_std)
...
parser.add_argument(
    "--zscore",
    choices=["none", "global", "cv_train"],
    default="none",
    help=(
        "Per-crop embedding z-score. "
        "'global': mu/std from all crops in pooled set (minor info leakage across CV folds). "
        "'cv_train': fit mu/std on each CV training fold only (recommended for unbiased CV)."
    ),
)
...
RESULTS_DIR = f"/home/tnocon/mil/abmil/results/{model_name}-{zscore_normalize}"
